...
async def created_order(
    user_client: AsyncClient, moderator_client: AsyncClient, setup_certification
):
    movie_data = {
        "name": "Payment Movie",
        "year": 2024,
...
async def test_create_checkout_session(user_client: AsyncClient, created_order: dict):
    with patch("stripe.checkout.Session.create") as mock_stripe:
        ...
        response = await user_client.post(
            f"/api/payments/create-checkout-session/{created_order['id']}"
        )
...
async def test_stripe_webhook_success(
    client: AsyncClient, created_order: dict, db_session
):
    ...
    with (
        ...
        response = await client.post(
            "/api/payments/webhook", content=b"payload", headers=headers
        )
        assert response.status_code == 200
        ...
        res = await db_session.execute(
            select(Order).where(Order.id == created_order["id"])
        )
